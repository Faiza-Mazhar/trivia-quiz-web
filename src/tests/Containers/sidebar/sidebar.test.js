...
test("should have fetched categories information and populate the category dropdown on loading the page", async () => {
  ...
  await act(async () => {
    ...
    expect(getByText(/Select Category/i).textContent).toBe(
      "Select CategoryAnyGeneral KnowledgeEntertainment: Books"
    );
  });
});
